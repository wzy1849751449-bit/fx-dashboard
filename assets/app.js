...
function renderTable(R, D) {
  ...
  // 日期跳转控件的可选范围，限制在有数据的区间内
  const jd = $("#jumpDate");
  if (jd && dates.length) { jd.min = dates[0]; jd.max = dates[dates.length - 1]; }

  $("#rateTable").innerHTML = `<div class="scroll" id="rateScroll"><table>
    ...
    <tbody>${[...R].reverse().map((r) => `<tr data-d="${r.d}"${dm[r.d] ? ' class="hasdeal"' : ""}>
...
/**
 * 跳到指定日期并高亮。
 * 周末、公众假期没有行情，用户选到这种日子时回退到之前最近的一个交易日，
 * 并如实说明落到了哪一天，不假装用户选的那天有数据。
 */
function jumpToDate(want) {
  const R = state.rates;
  if (!want || !R.length) return;
  const msg = $("#jumpMsg");
  let hit = R.filter((r) => r.d <= want).pop();
  if (!hit) {
    // 越界时把旧高亮清掉，否则会留着上一次的行让人以为跳成功了
    $$("#rateTable tr.hl").forEach((n) => n.classList.remove("hl"));
    msg.innerHTML = `<span class="warnink">${want} 早于现有数据（最早 ${R[0].d}）。</span>`;
    return;
  }
  const row = $(`#rateTable tr[data-d="${hit.d}"]`);
  if (!row) return;

  $$("#rateTable tr.hl").forEach((n) => n.classList.remove("hl"));
  row.classList.add("hl");

  const box = $("#rateScroll");
  // 让目标行落在可视区中部，而不是贴着顶边
  box.scrollTop = row.offsetTop - box.clientHeight / 2 + row.offsetHeight / 2;

  const deal = state.deals.find((x) => x.d === hit.d);
  msg.innerHTML = hit.d === want
    ? `已定位 <b>${hit.d}</b>${deal ? `　·　这天有议价 <b>${fmt0(deal.rate)}</b>` : ""}`
    : `<span class="warnink">${want} 没有行情（周末或假期）</span>，已跳到之前最近的 <b>${hit.d}</b>${deal ? `　·　这天有议价 <b>${fmt0(deal.rate)}</b>` : ""}`;
}
...
function bind() {
  ...
  // 日期跳转：选完即跳；「最新」回到最后一个交易日
  $("#jumpDate").onchange = (e) => jumpToDate(e.target.value);
  $("#jumpToday").onclick = () => {
    const last = state.rates[state.rates.length - 1];
    if (!last) return;
    $("#jumpDate").value = last.d;
    jumpToDate(last.d);
  };
  // 点表格里任意一行也高亮它，方便逐行核对
  $("#rateTable").addEventListener("click", (e) => {
    ...
    if (!tr) return;
    $("#jumpDate").value = tr.dataset.d;
    jumpToDate(tr.dataset.d);
  });
